...
async def index(request):
    ...
    def query(tr):
        ...
        questions = []
        for bindings in out:
            tags = [x['tag'] for x in tr.FROM('QADOM:QUESTION', bindings['key'], 'tag', h.var('tag'))]  # noqa
    ...
    now = int(time())
    context = dict(logged=request.logged, questions=questions, naturaldelta=naturaldelta, now=now)
